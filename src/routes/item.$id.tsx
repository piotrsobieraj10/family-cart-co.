...
function Inner({
  ...
  const removeItem = async () => {
    if (!canEditAnyItem(role) && it.created_by !== userId)
      return toast.error(
    ...
    >
          ...
          <div className="space-y-2">
            <input
              ...
              onChange={(e) => {
                ...
                setEditName(v);
                if (!categoryManual) setEditCategory(categorizeProduct(v));
              }}
          ...
          <>
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              ...
              <span className="text-foreground">
                {storesQ.data?.find((s) => s.id === it.store_id)?.name ??
        ...
        <div className="mt-3 text-xs text-muted-foreground space-y-1">
          ...
          {it.bought_at && (
            <div>
              ...
              {new Date(it.bought_at).toLocaleString(isEnglish ? "en-US" : "pl-PL")}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        {canChangeItemStatus(role) && it.status !== "unavailable" && it.status !== "bought" && (
          <button
            onClick={() => markStatus("unavailable")}
